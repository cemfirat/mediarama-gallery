...
final readonly class DbalPublicGalleryQuery implements PublicGalleryQuery
{
    ...
    public function collection(Uuid $id): ?PublicCollectionResult
    {
        $row = $this->connection->fetchAssociative(
            $this->publicCollectionsCte().$this->collectionSelect().'
            WHERE c.id = :id',
            ['id' => $id->toRfc4122()],
        );
...
ORDER BY cm.position ASC, m.captured_at DESC NULLS LAST, m.created_at ASC
...
SELECT EXISTS (
    ...
    JOIN public_collections c ON c.id = cm.collection_id
    WHERE m.id = :media
      AND m.deleted_at IS NULL
      AND m.processing_state = 'ready'
      AND m.moderation_state = 'published'
...
SQL,
    ...
    private function publicCollectionsCte(): string
    {
        return <<<'SQL'
WITH RECURSIVE public_collections AS (
    SELECT c.*
    FROM collections c
    ...
    JOIN public_collections parent ON parent.id = child.parent_id
